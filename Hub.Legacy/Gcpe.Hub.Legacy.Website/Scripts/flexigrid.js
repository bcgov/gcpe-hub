 ...
 * GCPE (Bruno) Patches:
 *  - send the hiddencolumns to the server.
 *  - allow loading of next pages without removing the previous ones (for infinite scrolling)
 */
(function ($) {
    /*
    ...
    $.addFlex = function (t, p) {
        ...
        var g = {
            ...
            fixHeight: function (newH) {
                ...
                //if (nd > newH) $(g.nDiv).height(newH).width(200); else //patch for the column show/hide popup to behave like the previous version. GCPE (Bruno)
            ...
            dragMove: function (e) {
                if (this.colresize) {//column resize
                    ...
                    // this line removed to fix sorting of columns (GCPE)
            ...
            addData: function (data) { //parse data
                    ...
                    $.each(data.rows, function (i, row) {
                        ...
                        $('thead tr:first th', g.hDiv).each( //add cell
                                function () {
                                    ...
                                    //KB : Adding model attribute
                                    $(td).attr("model", $(this).attr("model"));
                                    $(tr).append(td);
                                    td = null;
                                }
                        );
                ...
                var rows = $('tr', tbody); // Added GCPE (Bruno)
                if (p.newp == 1 || p.usepager) {
                    $('tr', t).unbind();
                    $(t).empty();
                    $(t).append(tbody);
                } else {
                ...
                this.addCellProp(rows);
                this.addRowProp(rows);
                this.rePosDrag();
                tbody = null;
                data = null;
                i = null;
            ...
            addCellProp: function (rows) {
                $('td', rows).each(function () { // patch GCPE (Bruno)
                    ...
                    if (pth != null) {
                        ...
                        } Do not change the background color of the rows of the sorted column. GCPE (Bruno)*/
                        $(tdDiv).css({
                            textAlign: pth.align,
            ...
            addRowProp: function (rows) {
                rows.on('click', function (e) { // patch GCPE (Bruno)
                    ...
                    // KB : Row Selection event
                    if (p.onRowSelected) {
                        p.onRowSelected($(this), g);
                    }
        ...
        var rows = $('tbody tr', g.bDiv); // Added GCPE (Bruno)
        g.addCellProp(rows);
        g.addRowProp(rows);
